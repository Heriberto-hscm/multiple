...
function PrintVerifyFailed()
{
	...
	$('#idModalNotificationText').text('Algunas preguntas tienen respuestas incompletas. Necesitas responder para continuar');
	$('#idModalNotification').modal('show');
}
...
function PrintAskConfirmOK(InitQuestion)
{
	...
	$('#idModalConfirmationText').text('Confirma su respuesta ?');
	$('#idModalConfirmation').modal('show');
}
...
function PrintAskVerifyQuest()
{
	...
	$('#idModalConfirmationText').text('¿Estás seguro de confirmar todas tus respuestas?');
	$('#idModalConfirmation').modal('show');
}
...
{
	PrintGlobalError('No se permite el error durante la carga de la página o la actualización de la página.');
}
...
function ShowQuestionMenu()
{
	...
	if (!options.questSlide && options.showListQstToAns)
	{
		...
		menu.html("debe responder<br />" + html);		
	}
}
